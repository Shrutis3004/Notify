...
var Joi = require('joi');
var supportedLanguages = require('../supported_languages.json');

var filterSchema = Joi.object.keys({
  field: Joi.string().valid('last_session', 'first_session', 'session_count', 'session_time', 'amount_spent', 'bought_sku', 'tag', 'language', 'app_version', 'location', 'email').required(),
...
var operatorSchema = Joi.object().keys({
  operator: Joi.string().valid('OR', 'AND').required()
});

var supportedLanguagesObj = {};
...
var buttonSchema = Joi.object().keys({
  id: Joi.string(),
  text: Joi.string(),
  icon: Joi.string()
});

var webButtonSchema = Joi.object().keys({
...
var contentSchema = Joi.object().keys(supportedLanguagesObj);
var headingsSchema = Joi.object().keys(supportedLanguagesObj);
var subtitleSchema = Joi.object().keys(supportedLanguagesObj);
